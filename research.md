...
Retrieval: Задача этапа отсеять большое количество нерелевантных кандидатов и передать топ-к потенциально релевантных кандидатов. Их порядок на этом этапе не важен. Recall@k - оценивает способность алгоритма охватить максимум потенциально релевантных кандидатов
Ranking: Задача этапа ранжировать выдачу предыдущего. NDCG@n - оценивает позицию кандидата в выдаче, позволяет использовать небинарную релевантность (наняли - 3 балла, открыли контакты - 2 балла, открыли резюме - 1 балл). Этап и метрика отражают поведение пользователя: изучение только нескольких первых страниц выдачи
